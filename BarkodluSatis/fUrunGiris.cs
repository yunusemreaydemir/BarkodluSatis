...
namespace BarkodluSatis
{
    public partial class fUrunGiris : Form
    {
        ...
        private void bKaydet_Click(object sender, EventArgs e)
        {
            if (tBarkod.Text != "" && tUrunAdi.Text != "" && cmbUrunGrubu.Text != "" && tAlisFiyati.Text != "" && tSatisFiyati.Text != "" && tKdvOrani.Text != "" && tMiktar.Text != "")
            {
                if (db.Urun.Any(a => a.Barkod == tBarkod.Text))
                {
                    ...
                    db.SaveChanges();
                    gridUrunler.DataSource = db.Urun.OrderByDescending(a => a.UrunId).Take(10).ToList(); // ürün id ye göre tersten sıralama
                ...
                else
                {
                    ...
                    gridUrunler.DataSource = db.Urun.OrderByDescending(a => a.UrunId).Take(20).ToList(); // ürün id ye göre tersten sıralama
        ...
        {
            if (tUrunAra.Text.Length >= 2) // iki karakterden sonra aramaya başla
            {
                string urunad = tUrunAra.Text;
                gridUrunler.DataSource = db.Urun.Where(a => a.UrunAd.Contains(tUrunAra.Text)).ToList(); // veritabanından kelimeye göre ürün arama 
                Islemler.GridDuzenle(gridUrunler);
            }
        }
        ...
        {
            fUrunGrubuEkle f = new fUrunGrubuEkle(); //ürün grubu formunu ekleme
            f.ShowDialog();
        }

        private void bBarkodOlustur_Click(object sender, EventArgs e) //8 karakterli standart 1 den başlayan barkod numarası oluşturduk
        {
        ...
        private void silToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (gridUrunler.Rows.Count > 0)
            {
                ...
                DialogResult onay = MessageBox.Show(urunad + " ürününü silmek istiyormusunuz", "Ürün Silme İşlemi", MessageBoxButtons.YesNo);
